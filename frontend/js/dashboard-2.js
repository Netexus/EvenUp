...
const getGroupId = (g) => g?.group_id ?? g?.id;
const getGroupName = (g) => g?.group_name ?? g?.name ?? 'Group';
const getMemberCount = (g) =>
  g?.num_members ?? g?.members_count ?? (Array.isArray(g?.members) ? g.members.length : undefined);
...
window.__GROUPS__ = window.__GROUPS__ || [];
...
window.showGroupDetailsById = (gid) => {
  const g = (window.__GROUPS__ || []).find(x => String(getGroupId(x)) === String(gid));
  if (!g) return console.warn('Group not found for id:', gid);
  // Llama a tu función existente:
  if (typeof showGroupDetails === 'function') {
    showGroupDetails(g);
  } else {
    console.warn('showGroupDetails(g) no está definida.');
  }
};

/**
 * Renderizes the grid of groups.
 * @param {HTMLElement|string} container 
 * @param {Array} groups - List of groups (each item may have id|group_id, name|group_name, num_members|members...).
 * @param {Object} [statsById] - dictionary { [groupId]: { totalExpenses, youOwe, youreOwed } }
 */

function renderGroupsGrid(container, groups, statsById = {}) {
  ...
  const cardsHTML = (groups || []).map((groupData) => {
    const gid = getGroupId(groupData);
    const name = getGroupName(groupData);
    const initials = name.trim().slice(0, 2).toUpperCase();
    const members = getMemberCount(groupData);

    // Stats: intenta primero en statsById; si no, mira campos comunes en groupData
    const stats = statsById[gid] || {};
    const totalExpenses = stats.totalExpenses ?? groupData.total_spent ?? groupData.totalExpenses ?? groupData.total;
    const youOwe       = stats.youOwe       ?? groupData.you_owe      ?? groupData.youOwe;
    const youreOwed    = stats.youreOwed    ?? groupData.you_are_owed ?? groupData.youreOwed;

    return `
      <div class="group-card" onclick="showGroupDetailsById('${gid}')">
        <div class="group-header">
          <div class="group-avatar">${initials}</div>
          <div class="group-info">
            <h3>${name}</h3>
            <p>${members !== undefined ? `(${members}) members` : ''}</p>
          </div>
        </div>
        <div class="group-stats">
          <div class="stat-item">
            <span class="stat-value">${fmtMoney(totalExpenses)}</span>
            <span class="stat-label">Total Expenses</span>
          </div>
          <div class="stat-item">
            <span class="stat-value">${fmtMoney(youOwe)}</span>
            <span class="stat-label">You Owe</span>
          </div>
          <div class="stat-item">
            <span class="stat-value">${fmtMoney(youreOwed)}</span>
...
async function loadAndShowGroupDetails(groupId) {
  try {
    ...
    let resp = await apiFetch(`/expense_groups/${groupId}?userId=${currentUserId}`);

    let groupInfo;
    if (Array.isArray(resp)) {
      groupInfo = resp.find(g => String(g.group_id ?? g.id) === String(groupId)) || resp[0] || {};
    } else {
      groupInfo = resp || {};
    }
    if (!groupInfo || (!groupInfo.group_id && !groupInfo.id)) {
    ...
    try {
      const balances = await apiFetch(`/settlements/group/${groupId}/user/${currentUserId}`);
      if (balances && Array.isArray(balances)) {
        for (const b of balances) {
          const val = Number(b.amount || b.net || 0);
